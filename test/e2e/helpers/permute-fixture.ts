...
export function permuteFixture(
  ...
  for (const params of getFixtureParameterPermutations(merged)) {
    const label = `[${params.bundler}][${params.platform}][${params.serveMode}][blazor=${params.blazor}]`;
    const runnable = supports(params.bundler, params.platform, params.serveMode, params.blazor);
