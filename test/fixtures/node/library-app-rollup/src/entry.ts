...
async function createLib() {
  ...
  return {
    greet: (name) => echo.Greet(name),
    add: (a, b) => echo.Add(a, b),
    boolNot: (value) => echo.BoolNot(value),
    pi: () => echo.Pi(),
    incrementCounter: () => { counter.Increment(); return counter.Value; },
    delayThenEcho: (value, delayMs) => asyncOps.DelayThenEcho(value, delayMs),
    boom: () => thrower.Boom(),
  };
}

// Initialize lib at module load time
export const lib = await createLib();
