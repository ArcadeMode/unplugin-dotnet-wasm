...
| J.1 | **Collision safety** — a generated `pkgName` writes into `node_modules/<pkgName>` and must **never** overwrite or delete a real dependency (e.g. the user's real `node_modules/vite`) | Slice 1 | ☐ **Actionable now.** Gate every write (and any future delete) on an **ownership marker** — a sentinel field in our generated `package.json`, or a `.dotnet-typeshim` sentinel file — so we only ever touch dirs we created. Doable on cold build (no cross-build memory needed): before writing/overwriting `node_modules/<pkgName>`, confirm it's absent or carries our marker; otherwise warn and skip. Independent of E | ☐ |
...
exports parked). **Actionable remaining:** C (PnP guard) and J.1 (collision safety) — both cold-build,
independent of the dev-server work. **Postponed:** E (live dev refresh), F (generate CLI), and J.2
