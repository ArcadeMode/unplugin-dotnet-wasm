...
| E | Live dev refresh (`configureServer` + manifest watch re-invokes `generate()`) | Slice 1 | **Parked** — deferred to the broader dev-server integration (needs the `configureServer`/watch plumbing that doesn't exist yet). Folded into the package README **Planned** → Watch/HMR item (type-shim regen rides the same manifest-change trigger). Generator instance already retained, so re-invocation is cheap once the seam lands | ⏸ |
...
F (generate CLI), J (sanitary node_modules), and A's default-export handling. **E (live dev refresh)
is parked** — deferred to the broader dev-server integration and tracked in the package README's
**Planned** section (Watch/HMR). D and E share one seam — cross-build state in the plugin-root
closure. D already lifted the shared state up (the `SourceFileChangeTracker` survives across builds
even though the generator is still re-instantiated per `buildStart`), so E's live refresh can reuse
that seam and layer on whenever the dev-server work begins. J (sanitary node_modules) builds on
D + E's in-memory ownership, so its dev/watch pruning is likewise gated on the dev-server integration
(cold-build pruning is independent).
