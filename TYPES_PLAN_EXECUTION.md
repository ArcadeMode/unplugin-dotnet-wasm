...
| C | Yarn PnP guard (detect `process.versions.pnp` → warn once → skip) | Slice 1 | ✅ **Implemented.** Pure helper `src/core/is-yarn-pnp.ts` (injectable `versions` param, unit-testable). Guard lives in `buildStart` (consumer-side, not in the generator) after `assetResolver` is constructed; warns every build and early-returns before emitter/generator setup. Only type-shim generation is skipped — `resolveId`, bundling, and asset resolution stay live. Tests: `src/core/is-yarn-pnp.test.ts` (5 cases: PnP present, absent, falsy). All 174 tests pass; collision-safety test (J.1) re-verified. | ✅ |
...
exports parked) → J.1 ✅ (collision safety) → C ✅ (PnP guard). **Actionable remaining:** none. **Postponed:** E (live dev refresh), F (generate CLI), and J.2
