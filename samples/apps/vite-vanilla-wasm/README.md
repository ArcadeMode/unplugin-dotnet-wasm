...
[`src/entry.ts`](src/entry.ts) is intentionally tiny:
...
- `typeshim` is a generated package shim: `[TSExport]` on
  [`WasmLibrary/Counter.cs`](../../libraries/WasmLibrary/Counter.cs) becomes a
  real TypeScript class you can `new` and call from the page.
...
```
samples/
├── libraries/WasmLibrary/   # shared .NET WebAssembly project (net10.0)
│   └── Counter.cs           # [TSExport] class consumed from JS
└── apps/vite-vanilla-wasm/  # Vanilla Vite + TypeScript host
