...
`webpack-vanilla-blazor`) lets Blazor own the router and pages, this sample does
the opposite: Vite owns the page, and Blazor only upgrades a few tags
(`<blazor-counter>`, `<blazor-date-time-now>`) into interactive components.
...
[`app/src/entry.ts`](app/src/entry.ts) starts Blazor explicitly — the SDK's
...
- Custom elements are registered in
  [`BlazorLibrary/Program.cs`](BlazorLibrary/Program.cs) with
...
- The tags live in plain [`index.html`](app/index.html) — any host that can
...
```
vite-vanilla-blazor/
├── BlazorLibrary/              # Blazor WASM library (net10.0)
