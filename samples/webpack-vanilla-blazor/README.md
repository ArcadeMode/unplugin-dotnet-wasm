...
Where [`sample-vite-blazor`](../sample-vite-blazor) embeds individual Blazor
components as custom elements inside a Vite page, this sample takes the opposite
route: the Blazor `Library` owns the UI (router, layout, pages), and Webpack is
just the host that builds, serves, and ships it.
...
[`app/src/index.ts`](app/src/index.ts) is intentionally tiny:
...
- CSS from the Library is imported in `styles.css` as bare paths
  (`css/app.css`, `Library.styles.css`); the plugin resolves them against the
...
- **Bootstrap via npm.** Bootstrap is a dependency of the Webpack app
  (`bootstrap@5.3.3`), imported from `app/src/styles.css`, instead of the
  template's `Library/wwwroot/lib` copy.
